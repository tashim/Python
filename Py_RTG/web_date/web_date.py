...
def set_define():
    global english,\
        ...
        txt_tab3_header,\
        txt_butt_lang, \
        txt_start_route,txt_tab4


    if not english:
        txt_tab4 = ' ניהול מסלול'
        txt_tab2 = 'מסלול'
        txt_tab3 = 'קורסים'
        ...
        txt_start_route = 'Start this route'
        txt_tab4 = 'Route manage'
...
def get_cbox_curs():
    try:
        db = dbMan.connnect()
        cur = db.cursor()
        cur.execute('SELECT * FROM courses where exist=1')
        db.close()
    except:
...
def db_select(frame,id,tab):
    ...
    for i in q:
        ...
        if type(i[1]) == type(' '):
            Label(frame,text=i[1], borderwidth=1, relief="ridge",font=font).grid(column=11,row=n)
        else:
...
def change_lang():
    ...
    tab_control.tab(3)['text'] = txt_tab4
    set_list_tab1(1)
    set_list_tab2(1)
    set_list_tab3()
    init4(True)
...
def focus(e):
    global mutex,FOCUS
    mutex = False
    if str(e.widget) == '.!notebook.!frame':
        # print('Tabb1')
        if not FOCUS=='.!notebook.!frame':
            FOCUS = str(e.widget)
            set_list_tab1(1)
        return

    if str(e.widget) == '.!notebook.!frame2':
        # print('Tabb2')
        if not FOCUS=='.!notebook.!frame2':
            FOCUS = str(e.widget)
            set_list_tab2(2)
        return

    if str(e.widget) == '.!notebook.!frame3':
        # print('Tabb3')
        if  FOCUS !='.!notebook.!frame3':
            FOCUS = str(e.widget)
            set_list_tab3()
        return

    if str(e.widget) == '.!notebook.!frame4':
        print('Tabb4')
        if not FOCUS=='.!notebook.!frame4':
            FOCUS = str(e.widget)
            # set_list_tab3()
            init4()

def init4(flag=False):
    if flag:
        for w in headerTab4.winfo_children(): w.destroy()
    if len(headerTab4.winfo_children())==0 or  flag:
        try:
            db = dbMan.connnect()
            cur = db.cursor()
            cur.execute('SELECT * FROM path;')
            ...
            init4()
            return
        Label(headerTab4,text=txt_tab4,font=font).pack()
        combo = Combobox(headerTab4,font=font)
        combo.pack()
        comboList = []
        comboListId = []
        comboListURL = []
        urlLable = ttk.Label(headerTab4,font=font)
        items={'id':comboListId,'combo':combo,'name':comboList,'url':comboListURL,'urlText':urlLable}
        urlLable.pack()
        combo.bind("<<ComboboxSelected>>", lambda index:onSelect4(items))
        q = cur.fetchall()
        for i in q:
            comboList.append(i[1])
            comboListId.append(i[0])
        ...
        onSelect4(items)
        # urlLable['text']=comboListURL[0]

    # f = Frame()
    # f.focus

class startCheck(Checkbutton):
    def __init__(self,frame,stat,id,n,**args):
        self.id = id
        self.var = IntVar()
        self.stat = stat
        self.n = n
        Checkbutton(frame,variable=self.var,command = self.cmd).grid(column=12,row=n)
        self.var.set(self.stat)
        self.bupdate=Button(frame,text='update',command = self.Update)
        # self.bupdate.grid(column=13, row=self.n)
        # self.bupdate.grid_forget()

    def cmd(self):
        print('cmd')
        if self.var.get() == self.stat:
            self.bupdate.grid_forget()
        else:
            self.bupdate.grid(column=13,row=self.n)
    def Update(self):
        ...
        try:
            db = dbMan.connnect()
            cur = db.cursor()
            cur.execute('UPDATE coursesperpath SET first=%s WHERE coursesPerPathCode=%s',( self.var.get(), self.id))
            db.commit()
            db.close()
            self.bupdate.grid_forget()
            self.stat = self.var.get()
        except:
            messagebox.showerror('Error', "\nCann`t READ from DB")
            exit(1)
...
def onSelect4(items):
    print(items['combo'].current())
    items['urlText']['text'] = items['url'][items['combo'].current()]
    q_select=\
    """
    SELECT  coursesPerPathCode,first, coursesperpath.courseCode, courseName
    FROM coursesperpath
    inner join courses on coursesperpath.courseCode=courses.courseCode 
      where   coursesperpath.pathCode=%s
    """
    try:
        db = dbMan.connnect()
        cur = db.cursor()
        cur.execute(q_select,items['id'][items['combo'].current()])
        db.close()
    except:
        messagebox.showerror('Error', "\nCann`t READ from DB")
        exit(1)
    n=10
    for w in frameTab4.winfo_children():
        w.destroy()
    Label(frameTab4, text=' ', font=font).grid(column=10, row=4)
    Label(frameTab4, text=txt_name_curs, font=font ,borderwidth=1, relief="ridge").grid(column=10, row=5)
    Label(frameTab4, text=txt_start_route, font=font, borderwidth=1, relief="ridge").grid(column=12, row=5)
    for i in cur.fetchall():
        Label(frameTab4,text=i[3],font=font).grid(column=10,row=n)
        # Label(frameTab4,text=i[1],font=font).grid(column=11,row=n)
        startCheck(frameTab4,i[1],i[0],n)
        n +=1

if __name__ == '__main__':

    mutex = False
    FOCUS = '.!notebook.!frame'
    ...
    langB = Button(root,text=txt_butt_lang,height=1,command=change_lang)
    ...
    tab_control.add(tab4, text=txt_tab4)# change text
    canvas4 = Canvas(tab4, borderwidth=2)
    fourFrame = Frame(canvas4)
    vsby4 = Scrollbar(tab4, orient="vertical", command=canvas4.yview)
    vsbx4 = Scrollbar(tab4, orient=HORIZONTAL, command=canvas4.xview)
    canvas4.configure(yscrollcommand=vsby4.set)
    canvas4.configure(xscrollcommand=vsbx4.set)
    fourFrame.bind("<Configure>", lambda event, canvas=canvas4: onFrameConfigure(canvas4))
    vsby4.pack(side="right", fill="y")
    vsbx4.pack(side="bottom", fill="x")
    canvas4.pack(side="left", fill="both", expand=True)
    canvas4.create_window((4, 4), window=fourFrame, anchor="ne")
    headerTab4 = Frame(fourFrame)
    headerTab4.pack(fill=BOTH)
    frameTab4 = Frame(fourFrame)
    frameTab4.pack()
    FourItems = {}

    tab1.bind("<FocusIn>",focus)
    tab2.bind("<FocusIn>",focus)
    tab3.bind("<FocusIn>",focus)
    tab4.bind("<FocusIn>",focus)
