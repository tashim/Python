...
if __name__ == '__main__':
    ...
    cur = dbsm.cursor()
    cur.execute("select b_entryID from Individual order by b_entryID desc limit 3")

    rez = cur.fetchall()
    print(rez)

    data['UniqID'] = rez[-1][0]
    # data['UniqID'] = '1115518188'
    print(data['UniqID'])
    log(SERVER_NAME)
    insert = 0
    update = 0
        ...
        for r in rez:
            if "phone_mobile" in r:
                ...
                if e:
                    ...
                    if len(e) > 1:
                        delete += len(e[1:])
                        log(str(delete) + "  delete " + str(e[1:]) + " cl_id " + str(r["client_id"]))
                        print("delete", delete, e[1:])
                else:
                    s = """
                    ...
                    try:
                        ...
                        cur.execute(s)
                        dbsm.commit()
                        log("  insert " + str(insert) + str(t) + " cl " + str(r["client_id"]))
                        print(insert, t)
                    except:
                        log(str(dbsm.Error) + "\n" + s)
                        print("error ",insert, t)
                    insert += 1
                    print(insert, t)
