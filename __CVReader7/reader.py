...
def main_fun(service):
    ...
    for fileName in os.listdir(file_path):
        ...
        if os.path.isdir(file_path + fileName):
             continue
        if fileName[-4:][0]=='.':
            if fileName == str(n) + fileName[-4:]:
                if os.path.isfile(file_path + str(n) + fileName[-4:]):
                    os.remove(file_path + str(n) + fileName[-4:])
                continue
            if os.path.isfile(file_path + str(n)+fileName[-4:]):
                os.remove(file_path + str(n)+fileName[-4:])
            copyfile(file_path + fileName, file_path + str(n)+fileName[-4:])
            fileName = str(n)+fileName[-4:]
        if fileName[-5:][0]=='.':
            if fileName == str(n) + fileName[-5:]:
                if os.path.isfile(file_path + str(n) + fileName[-5:]):
                    os.remove(file_path + str(n) + fileName[-5:])
                continue
            if os.path.isfile(file_path + str(n)+fileName[-5:]):
                os.remove(file_path + str(n)+fileName[-5:])
            copyfile(file_path + fileName, file_path + str(n)+fileName[-5:])
            fileName = str(n) + fileName[-5:]
        print(fileNameOld)
